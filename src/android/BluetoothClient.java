...
public class BluetoothClient extends CordovaPlugin {
    ...
    /* 开启服务 */
    public void startService(String message, CallbackContext callbackContext)
    {
        LOG.d(TAG, "start service");
        int count = 0;
        // If there are paired devices, add each one to the ArrayAdapter
        if (pairedDevices.size() > 0 && !message.trim().equals("")) {
            for (BluetoothDevice device : pairedDevices) {
				// start service listening
                if(message.equals(device.getName())){
                    LOG.d(TAG, "device info: "+ device.getName() + "  " + device.getAddress());
                    // Get the BluetoothDevice object
                    BluetoothDevice deviceRemote = mBtAdapter.getRemoteDevice(device.getAddress());
                    mChatService.connect(deviceRemote);
                    callbackContext.success("finished");
                    //count number add
                    count++;
                }
            ...
            if(count == 0){
                callbackContext.error("no compiled");
            }
        } else {
			Toast.makeText(context, "no bounded device", Toast.LENGTH_SHORT).show();
            callbackContext.success("no bounded");
        }
    }
    ...
    private void serverState(CallbackContext callbackContext) {
        ...
        if (D) LOG.d(TAG, "server state " +  mChatService.getState());
        /* 返回服务器状态 */
        callbackContext.success(this.getServerStateJSON());;
    }
...
class BluetoothChatService {
    ...
    private synchronized void setState(int state) {
        ...
        /* 链接并输出内容--触发状态改变条件  */
        mHandler.obtainMessage(BluetoothClient.MESSAGE_STATE_CHANGE, state, -1, null)
                .sendToTarget();
    }
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                LOG.e(TAG, "mmSocket connect failed reason: ", e);
                connectionFailed();
                return;
            }
